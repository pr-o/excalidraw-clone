...
**Date:** 2026-07-10
**Status:** Approved (amended same day: the `Arrowhead` union has **10** kinds, not 8 — `circle_outline` and `diamond_outline` were missed in the original survey; scope decision "all schema kinds, no dead values" extends to all 10)
...
`packages/scene/src/types.ts` defines 10 kinds (`arrow`, `bar`, `dot`, `circle`,
`cross`, `triangle`, `diamond`, `triangle_outline`, `circle_outline`,
`diamond_outline`) and every linear element carries nullable
`startArrowhead` / `endArrowhead` fields — but:
...
- Render **all 10 schema kinds** — no dead schema values remain, and no two
  kinds may render identically.
- Picker exposes **None** (null) plus the 10 kinds, for **both Start and End**.
...
Pure function; a switch over the 10 kinds. Geometry is derived from the tip
point and the angle from `prev` toward `tip` (`Math.atan2`), reusing the
existing constants `ARROWHEAD_LENGTH = 20` (below: L) and
`ARROWHEAD_ANGLE = Math.PI / 6` (moved into this module).
...
| `bar`              | segment perpendicular to shaft, centered at tip, total length L             | stroke only                |
| `dot`              | small circle centered on tip, diameter 0.6 · L                              | solid fill in stroke color |
| `circle`           | circle centered on tip, diameter 0.8 · L                                    | solid fill in stroke color |
| `circle_outline`   | same circle as `circle`                                                     | stroke only                |
| `cross`            | X of two segments centered on tip, each of total length L, at ±45° to shaft | stroke only                |
| `diamond`          | rhombus via `polygon`, far vertex at tip                                    | solid fill in stroke color |
| `diamond_outline`  | same rhombus                                                                | stroke only                |

(`dot`/`circle`/`circle_outline` follow upstream Excalidraw semantics: `dot` is
the small legacy filled circle; `circle` is filled, `circle_outline` outline.)

Filled kinds pass `fillStyle: "solid"` with `fill` set to the stroke color via
conditional assignment (the tsconfig's `exactOptionalPropertyTypes` rejects
`fill: opts.stroke` when `stroke` may be undefined).
...
- Two rows inside one `<Section label={t("properties.arrowheads")}>`:
  Start and End. Each row: 11 compact buttons — None + 10 kinds.
...
`properties.arrowhead_none`, and `properties.arrowhead_<kind>` for the 10 kinds.
...
- **Renderer** (`packages/renderer/test/arrowheads.test.ts`, new): for each of
  the 10 kinds — non-empty drawables, expected rough shape (`linearPath` /
